...
class SequentialWorkflowTest(unittest.TestCase):
    ...
    def test_actions_reuse_scf_and_saved_settings(self):
        with patch('calculation_workflow.run_scf', side_effect=self.fake_scf) as scf, \
            ...
            self.assertFalse(app.exception)
            self.assertTrue(self.button(app, 'Calculate dipole moment').disabled)
            ...
            self.assertFalse(app.exception)
            self.assertNotIn('scf_result', app.session_state)
            self.assertFalse(any(b.label == 'Calculate forces' for b in app.button))
            self.assertEqual(scf.call_count, 1)
            self.button(app, '🚀 Run DFT Calculation').click().run()
            self.assertFalse(app.exception)
    ...
    def test_basis_change_clears_results_and_error_output(self):
        with patch('calculation_workflow.run_scf', side_effect=self.fake_scf) as scf:
            app = AppTest.from_file('Home.py').run(timeout=30)
            self.button(app, '🚀 Run DFT Calculation').click().run()
            app.session_state.scf_error = ('old error', 'old output')
            basis = next(w for w in app.selectbox if w.label == 'Basis Set:')
            basis.select('6-31G').run()
            self.assertFalse(app.exception)
            self.assertNotIn('scf_result', app.session_state)
            self.assertNotIn('scf_error', app.session_state)
            self.assertFalse(any(h.value == '2. SCF Results' for h in app.header))
            self.assertFalse(any(e.label in ('Input Script', 'Calculation Output') for e in app.expander))
            self.assertEqual(scf.call_count, 1)
            self.button(app, '🚀 Run DFT Calculation').click().run()
            self.assertFalse(app.exception)
            self.assertEqual(app.session_state.scf_result['settings']['basis_set'], '6-31G')
            self.assertEqual(scf.call_count, 2)
    ...
    def test_failed_action_can_retry_without_losing_scf(self):
        with patch('calculation_workflow.run_scf', side_effect=self.fake_scf) as scf, \
            ...
            self.assertFalse(self.button(app, 'Calculate dipole moment').disabled)
            self.assertEqual(app.session_state.scf_result['actions'], [])
            self.button(app, 'Calculate dipole moment').click().run()
            self.assertFalse(app.exception)
            self.assertTrue(self.button(app, 'Calculate dipole moment').disabled)
